  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (submissionIds.length === 0)
        return { subs: [], entriesBySub: {}, itemsById: {}, catsById: {}, filledByUsers: {} };
      ...
      const items = itemIds.length
        ? await supabase
            ...
            .select("id, label, category_id, parent_item_id")
            .in("id", itemIds)
      ...
      const filledByIds = Array.from(
        new Set((entries.data ?? []).map((e: any) => e.filled_by).filter(Boolean)),
      );
    ...
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        ...
        {data && data.subs.length === 0 && (
          <p className="py-8 text-center text-sm text-muted-foreground">No entries for this day.</p>
        )}
                ...
                <div className="flex items-center justify-between gap-2 mb-2">
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground uppercase tracking-wider">
                      {s.checklist_templates?.cadence} ·{" "}
                      {s.checklist_templates?.format?.replace("_", " ")}
                    </div>
                  </div>
                  {s.status === "submitted" ? (
                    ...
                    <div className="space-y-1.5">
                      {list.map((e: any) => {
                        ...
                        return (
                          <div
                            key={e.id}
                            className="flex items-start gap-2 text-sm border-b pb-1.5 last:border-0"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {e.photo_url && (
                                <img src={e.photo_url} alt="" className="mt-1 rounded max-h-24" />
                              )}
                              <div className="text-[10px] text-muted-foreground mt-0.5">
                                by {filler?.full_name || filler?.email || "unknown"} ·{" "}
                                {new Date(e.filled_at).toLocaleTimeString([], {
                                  hour: "2-digit",
                                  minute: "2-digit",
                                })}
                              </div>
